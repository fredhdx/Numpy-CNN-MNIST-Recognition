...
def conv_layer_forward(input_data, layer, param):
    ...
    col_data = im2col_conv_batch(input_n, layer, h_out, w_out)
    col_data = col_data.reshape(k*k*c, h_out*w_out, batch_size, order='F')

    for b in range(batch_size):
        col_data_b = col_data[:, :, b]
        col_result = np.dot(col_data_b.T, param['w']) + param['b']
        output['data'][:, :, :, b] = col_result.reshape(h_out, w_out, num, order='F')
